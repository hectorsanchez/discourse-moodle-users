...
export default {
  name: "moodle-users-page",
  initialize() {
    withPluginApi("0.8.31", api => {
      // Detectar cuando estamos en /moodle/users y mostrar la interfaz
      api.onPageChange(() => {
        if (window.location.pathname === "/moodle/users") {
          // Ocultar el contenido normal de Discourse
          const mainContent = document.querySelector('.main-content');
          if (mainContent) {
            mainContent.style.display = 'none';
          }
          
          // Mostrar nuestra interfaz de usuarios de Moodle
...
function showMoodleUsersInterface() {
  ...
  container.className = 'moodle-users-page';
  container.innerHTML = `
